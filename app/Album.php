...
class Album extends Model {
    ...
    {
        $highestPlaysCount = $this->tracks->pluck('plays_count')->max();
        $settings = app(Settings::class);
        $this->tracks->map(function (Track $track) use($highestPlaysCount, $settings) {
            if ($settings->get('artist_provider') === 'spotify' && $track->spotify_popularity) {
                $track->popularity = $track->spotify_popularity;
            } else if ($highestPlaysCount) {
                $track->popularity = $track->plays_count / ($highestPlaysCount * 50);
            } else {
                $track->popularity = 50;
            }
            return $track;
